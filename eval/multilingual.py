...
def signal(language: Literal["de", "id"], quantity: int, t0: datetime) -> Item:
    ...
    if language == "de":
        ...
        return Item(
            "eval-multilingual", "EMAIL", "ACCEPTED",
            {"eval-multilingual.eml": _email(
                t0, 32, KRIEGER, f"Bestellung {po}",
                f"Guten Tag, Bestellung {po}: Menge {phrase} ist versandbereit.",
                pdf=("eval-german.pdf", pdf),
            )},
            po=po, material="MAT-48219",
        )
    media = "990000000000001"
    return Item(
        "eval-multilingual", "WHATSAPP", "ACCEPTED",
        {
            "eval-multilingual.json": _whatsapp(
                t0, 8, KRIEGER[2], KRIEGER[0],
                f"Pesanan {po}, jumlah {phrase} siap dikirim", media,
            ),
            f"media/{media}.png": render(
                [f"PESANAN {po}", f"JUMLAH {phrase}"], seed=quantity,
            ),
        },
        po=po, material="MAT-48219",
    )
...
class Ocr:
    ...
    def analyze_document(self, **request: Any) -> dict[str, Any]:
        ...
        return {"Blocks": [
            {"Id": "line", "BlockType": "LINE",
             "Text": f"{word} {self.quantity} {unit}"},
            {"Id": "word-qty", "BlockType": "WORD", "Text": str(self.quantity),
             "Confidence": 71.0},
            {"Id": "word-unit", "BlockType": "WORD", "Text": unit,
             "Confidence": 83.0},
        ]}
