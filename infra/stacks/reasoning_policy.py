...
def add_reasoning_policy(stack: Stack, env_name: str) -> str:
    ...
    rules = [
        rule(
            id="BR05AUTO0001",
            expression=(
                "(tier = 1) => (total_cost_cents <= auto_limit_cents "
                "and confidence >= confidence_min and all_reversible)"
            ),
        ),
        rule(id="BR07DONOR001", expression=("(tier = 1 or tier = 2) => donor_protection_ok")),
        rule(
            id="BR12RAR00001",
            expression=("(tier = 1 or tier = 2) => total_cost_cents < rar_protected_cents"),
        ),
    ]
