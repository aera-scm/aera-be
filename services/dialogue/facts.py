...
from services.notifier.handler import partner_emails
...
def load_facts(cases: CaseStore, sap: SapClient, case_id: str) -> SupplierFacts:
    case = cases.get(case_id)
    if case is None or case.status is not CaseStatus.INVESTIGATING or not case.po_number:
        raise ValueError("supplier question requires an investigating case with an open PO")
    po = sap.get(
